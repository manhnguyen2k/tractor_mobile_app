...
class LineChart2 extends StatefulWidget {
  LineChart2({required this.tractorId, required this.token});
          ...
          setState(() {
            ...
            if (dataPoints1.length >= maxLength) {
              ...
              for (int i = 0; i < dataPoints1.length; i++) {
                dataPoints1[i] = FlSpot(i.toDouble(), dataPoints1[i].y);
              }
            }
            if (dataPoints2.length >= maxLength) {
              ...
              for (int i = 0; i < dataPoints2.length; i++) {
                dataPoints2[i] = FlSpot(i.toDouble(), dataPoints2[i].y);
              }
            }
            if (dataPoints3.length >= maxLength) {
              ...
              for (int i = 0; i < dataPoints3.length; i++) {
                dataPoints3[i] = FlSpot(i.toDouble(), dataPoints3[i].y);
              }
            }
        ...
        leftTitles: AxisTitles(
          sideTitles: SideTitles(
            ...
            getTitlesWidget: (value, meta) {
              return Text(
                value.toInt().toString(),
                style:const TextStyle(color: AppColors.text_dark, fontSize: 12),
              );
            },
          ),
        ),
